...
public class GuidanceSystem : MonoBehaviour
{
    ...
    public string GetApparatusGuidance(ApparatusResult result)
    {
        ...
        if (!result.IsVernierPresent || !result.IsScrewGaugePresent)
            return "Ensure both instruments are ready before starting.";
        ...
        if (!result.IsCorrectScale)
            return "Select the correct scale: mm for caliper, 0.01mm for screw gauge.";
